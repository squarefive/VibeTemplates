...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check codebase map document format.")
    parser.add_argument(
        "targets",
        nargs="*",
        help="Files or directories to check. Defaults to docs/architecture/codebase-map.md.",
    )
    ...
    for path in unique_files:
        checked += 1
        path_errors = check_codebase_map_doc(path)
        if path_errors:
